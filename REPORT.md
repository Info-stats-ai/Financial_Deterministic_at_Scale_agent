...
a test fixture, not production replay. Curated evidence includes success, not-found, and a
bad-login hard failure with masked screenshot. A genuine discovery directory must still be
created with the submitter's Anthropic key; simulated tests are explicitly not represented
as that evidence.
...
A genuine Anthropic discovery directory is generated by the README command when
`ANTHROPIC_API_KEY` is present; this environment did not contain that key, so live
discovery evidence is not fabricated.
